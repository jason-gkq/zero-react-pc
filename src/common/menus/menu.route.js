...
export function generateRoute(menuList = [], permList = null) {
  ...
  const routeList = require.context("@src/pages", true, /index\.route\.js/);
  ...
  const routes = routeList.keys().map((path) => {
    ...
    if (routeProps && routeProps.path) {
      ...
      if (!existRoute[key]) {
        ...
        console.error(
          `警告：【${name}-${key}】路由无效！已存在相同的路由【${m.name}-${m.routeProps.path}】，请在index.route.js中重置routeProps.path！`
        );
      }
    } else {
      console.error(
        `警告：【${name}】路由无效！routeProps，routeProps.path不能为空！`
      );
    }
  });
  ...
  const filterMenus = (item) => {
    ...
    if (id) {
      if (!valid) {
        ...
        console.error(
          `警告：【${name}-${path}】菜单无效！已存在相同的路径菜单【${m.name}-${m.path}】，请在menu.data.js中重置path！`
        );
      }
    }
