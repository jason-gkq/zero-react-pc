...
// 查询参数详细
const getConfig = async (configId: number[] | number): Promise<{ data: IResGetConfig['data'] }> => {
...
// 修改参数
const updateConfig = async (data: IReqUpdateConfig): Promise<IResUpdateConfig> => {
  return await HttpClient.put('system/config', data)
}
// 删除参数
const delConfig = async (configId: number | number[]): Promise<IResDelConfig> => {
  return await HttpClient.delete(`system/config/${configId}`)
}

// 导出参数
const exportConfig = async (data: IReqExportConfig): Promise<IResExportConfig> => {
  return await HttpClient.get(`system/config/export`)
}
